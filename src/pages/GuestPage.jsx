...
export default function GuestPage() {
    ...
    const getCurrentStepIndex = (status) => {
        ...
        if (lowerStatus.includes("terima") || lowerStatus.includes("pending") || lowerStatus.includes("baru")) return 0;
        if (lowerStatus.includes("cuci") || lowerStatus.includes("proses") || lowerStatus.includes("progress")) return 1;
        if (lowerStatus.includes("setrika") || lowerStatus.includes("gosok") || lowerStatus.includes("siap")) return 2;
        if (lowerStatus.includes("selesai") || lowerStatus.includes("ambil") || lowerStatus.includes("done")) return 3;
    ...
    const handleTracking = async (e) => {
        ...
        try {
            console.log(`Menghubungkan Guest ke Database Admin... Mencari Nota: ${searchInvoice}`);
            ...
            if (error) {
                ...
                setResult({
                    ...
                    customer: data.customer_name || data.customer || data.nama_pelanggan || "Pelanggan BILAS",
        ...
        <div className="min-h-screen bg-[#0B132B] text-white flex flex-col">
            ...
            <nav className="border-b border-gray-800 bg-[#1C2541]/50 backdrop-blur-md px-6 py-4 flex justify-between items-center sticky top-0 z-50">
                ...
                <div className="flex gap-3">
                    ...
                    <Link to="/register" className="bg-[#00E676] hover:bg-[#00C853] text-white text-sm font-bold px-4 py-2 rounded-xl transition shadow-lg shadow-green-500/10">
                        Daftar Akun
                    </Link>
                </div>
            </nav>
            ...
            <main className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-3xl mx-auto w-full my-8">
                ...
                <p className="text-gray-400 text-sm max-w-lg mb-8">
                    Masukkan nomor nota / ID transaksi yang diberikan oleh kasir/admin untuk melacak progress pakaian Anda.
                </p>

                {/* FORM PENCARIAN */}
                <form onSubmit={handleTracking} className="w-full max-w-md bg-[#1C2541] p-2 rounded-2xl flex items-center border border-gray-800 shadow-xl mb-8">
                    <input
                        type="text"
                    ...
                    >
                        <BiSearchAlt className="text-lg me-1" /> {loading ? "Mencari..." : "Track"}
                    </button>
                </form>
                ...
                {result && (
                    <div className="w-full max-w-xl p-6 rounded-2xl bg-[#1C2541] border border-gray-800 text-left shadow-2xl mb-12 animate-fadeIn">
                        <div className="flex justify-between items-start border-b border-gray-800 pb-3 mb-4">
                            <div>
                                <h3 className="text-xs text-gray-400 uppercase font-bold tracking-wider">ID Transaksi / Nota</h3>
                                <p className="font-mono text-xs font-bold text-blue-400 break-all">{result.id}</p>
                            </div>
                            <div className="text-right">
                                <h3 className="text-xs text-gray-400 uppercase font-bold tracking-wider">Nama Pelanggan</h3>
                                <p className="text-sm font-bold text-gray-200">{result.customer}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-6 py-2 overflow-x-auto">
                            {result.steps.map((step, index) => (
                                <div key={index} className="flex-1 text-center relative min-w-[65px]">
                                    ...
                                    <div className={`w-8 h-8 mx-auto rounded-full flex items-center justify-center text-xs font-black z-10 relative shadow-md transition-all duration-300
                                        ${index <= result.currentStep ? "bg-[#00E676] text-[#0B132B]" : "bg-gray-700 text-gray-400"}`}>
                                        {index + 1}
                                    </div>
                                    <p className={`text-[10px] mt-2 font-bold tracking-wide ${index <= result.currentStep ? "text-[#00E676]" : "text-gray-500"}`}>
                                        {step}
                                    </p>
                                </div>
                            ))}
                        ...
                        <div className="bg-[#131A30] p-3 rounded-xl border border-gray-800 flex justify-between items-center">
                            <span className="text-xs font-bold text-gray-400 uppercase">Status Update Kasir:</span>
                ...
                {hasSearched && !result && !loading && (
                    <div className="w-full max-w-md p-4 mb-8 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-semibold">
                        ⚠️ ID Transaksi tidak ditemukan. Pastikan data sudah disimpan oleh Admin di panel dashboard.
                    </div>
                )}
                ...
                <div className="w-full max-w-2xl bg-[#1C2541]/50 border border-gray-800 rounded-2xl shadow-2xl p-6 text-left mt-4">
                    <div className="flex items-center gap-2 mb-4 border-b border-gray-800 pb-3">
                        ...
                        <h2 className="text-lg font-bold text-gray-200 tracking-wide">Daftar Layanan & Harga / Kg</h2>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-800/60 text-sm">
                                {servicesPriceList.map((service, index) => (
                                    <tr key={index} className="hover:bg-[#1C2541]/30 transition duration-150 group">
